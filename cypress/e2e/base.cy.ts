...
describe("User Registration and Flow", () => {
  ...
  it("should sign up, sign in, logout and interact with book", () => {
    ...
    cy.get(":nth-child(2) > .form-control").type(randomName); // email
    cy.get(":nth-child(3) > .form-control").type(randomSurname); // password
    cy.get(":nth-child(4) > .form-control").type(randomEmail);
    cy.get(":nth-child(5) > .form-control").type(randomPassword);
    cy.get(":nth-child(6) > .form-control").type(randomUrl);
...
describe("Borrow book", () => {
  ...
  it("sign in and borrow book", () => {
    ...
    cy.get(".btn").contains("Borrow this book").click();
    cy.get(".btn").contains("Borrow this book").should("have.class", "disabled");
  });
});
